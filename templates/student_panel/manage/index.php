...
if(isset($_POST['update'])){
    ...
    $ln = ucwords($_POST['lname']);
    $fn = ucwords($_POST['fname']);
    $col = $_POST['college'];
    $cour = $_POST['course'];
    $yr = $_POST['year'];
    $student = $db->search_student($_SESSION['search']);
    $oldStudNo = $student['stud_no'];
    $db->update_student($_SESSION['search'], $studNo, $ln, $fn, $yr, $col, $cour, $oldStudNo);
}
...
if(isset($_POST['delete'])){
    ...
    if($confirm){
        $student = $db->search_student($_SESSION['search']);
        if(isset($student)){
            $studNo = $student['stud_no'];
            $db->delete_student($studNo);
            
            $path = "../img/known/{$studNo}.jpeg";
            if(file_exists($path)){
                unlink($path);
                echo "<script> alert('\'{$studNo}\' has been deleted') </script>";
            }
        }
    }
    else {
            ...
            <table class="alternating-row-table">
                ...
                <?php 
                 ...
                 while($row = mysqli_fetch_array($run)){
                     ...
                     $name = "{$lname}, {$fname}";
                ?>
                <tr id='info' onclick="click_student('<?php echo $row['stud_no'];?>')">
